...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (@Medication_top_code)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (@Medication_top_code)
...
WHERE C.drug_exposure_start_date >= DATEFROMPARTS(2019, 1, 1)
